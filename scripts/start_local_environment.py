...
class LocalEnvironmentManager:
    ...
    def _start_docker_services(self) -> bool:
        ...
        try:
            ...
            self.logger.error("Docker or docker-compose not found. Please install Docker Desktop.")
            return False
        except Exception as e:
    ...
    def _wait_for_service(self, name: str, host: str, port: int, 
                         endpoint: str = None, timeout: int = 60) -> bool:
    ...
    def _initialize_test_data(self, cache_client, message_client, db_client):
        ...
        test_doc_id = db_client.insert_one("test_collection", {
            "name": "local_environment_test",
            "status": "initialized",
            "timestamp": time.time()
        })
        
        self.logger.info(f"Initialized test data - Cache: ✓, Kafka: ✓, MongoDB: ✓ (doc: {test_doc_id})")
    ...
    def _display_environment_info(self):
        ...
        print("\n🔗 Service Access URLs:")
        print("   📊 Grafana Dashboard:    http://localhost:3000 (admin/netskope_grafana_2024)")
        print("   📈 Prometheus Metrics:   http://localhost:9090")
        print("   🔍 Jaeger Tracing:       http://localhost:16686")
        print("   ☁️  LocalStack Console:   http://localhost:4566")
...
def main():
    ...
    parser.add_argument("--start", action="store_true", help="Start the local environment")
    parser.add_argument("--stop", action="store_true", help="Stop the local environment")
    parser.add_argument("--restart", action="store_true", help="Restart the local environment")
    ...
    if args.stop:
        ...
        if success:
            ...
            try:
                ...
                print("   Use 'python scripts/start_local_environment.py --stop' to shut down")
