...
try:
    ...
    # Try with src prefix if running from project root
...
def cmd_environment(args):
    ...
    if args.env_action == 'detect':
        ...
        env_name = target_env.value if target_env else env_manager.get_current_environment().value
    ...
    if args.service_action == 'health':
        ...
        if service_type in ['api', 'all']:
            ...
            try:
                ...
                print(f"  GET events: Failed (expected in mock mode)")
...
def cmd_staging(args):
    ...
    try:
        ...
        deployment = StagingDeployment(
            ...
            namespace=args.namespace or "netskope-staging"
        )
        
        if args.staging_action == 'deploy':
            print("🚀 Deploying Staging Environment (E4)...")
            print("⚠️  This is a production-like environment with enhanced security")
            ...
            if args.service:
                ...
                cmd = [
                    "kubectl", "logs", 
                    "-n", args.namespace or "netskope-staging",
            ...
            print("⚠️  Staging tests may take longer due to enhanced security and HA setup")
            ...
            cmd = [
                ...
                "--rm", "-i", "--restart=Never",
                "--", "/app/run_staging_tests.sh", test_type
            ]
...
def cmd_production(args):
    ...
    try:
        ...
        if args.production_action == 'health-check':
            ...
            output_file = args.output or f"reports/production_health_report_{int(time.time())}.json"
            ...
            print(f"Mode: {'READ-ONLY' if health_data['read_only_mode'] else 'UNKNOWN'}")
            print(f"Overall Health: {health_data['overall_health']['status']}")
            print(f"Health Score: {health_data['overall_health']['health_percentage']:.1f}%")
            print(f"Services: {health_data['overall_health']['healthy_services']}/{health_data['overall_health']['total_services']} healthy")
    ...
    try:
        ...
        if args.integration_action == 'deploy':
            ...
            if args.service:
                ...
                cmd = [
                    "kubectl", "logs", 
                    "-n", args.namespace or "netskope-integration",
                    "-l", f"app={args.service}",
                    "--tail=100"
                ]
                if args.follow:
            ...
            try:
                ...
                print(f"📊 Monitor with: kubectl logs -n {args.namespace or 'netskope-integration'} job/{job_name} -f")
                return 0
            except subprocess.CalledProcessError:
...
Examples:
  ...
  netskope-sdet test integration -e local    # Run integration tests in local env
    ...
    env_parser.add_argument('env_action', choices=['detect', 'info', 'validate', 'set', 'list'],
    ...
    service_parser.add_argument('service_action', choices=['health', 'info', 'test'],
                               help='Service action to perform')
    service_parser.add_argument('service_type', nargs='?',
    ...
    local_parser.add_argument('local_action', choices=['start', 'stop', 'restart', 'status'],
                             help='Local environment action')
    
    # Integration environment commands
    integration_parser = subparsers.add_parser('integration', help='Integration environment management (Kubernetes)')
    integration_parser.add_argument('integration_action', 
    ...
    integration_parser.add_argument('--namespace', default='netskope-integration', help='Kubernetes namespace')
    integration_parser.add_argument('--service', help='Service name for logs (redis, kafka, mongodb, etc.)')
    integration_parser.add_argument('--follow', '-f', action='store_true', help='Follow logs')
    integration_parser.add_argument('--test-type', choices=['integration', 'e2e', 'security', 'performance', 'smoke'],
                                   help='Type of tests to run')
    
    # Staging environment commands
    staging_parser = subparsers.add_parser('staging', help='Staging environment management (Kubernetes HA)')
    staging_parser.add_argument('staging_action', 
                               choices=['deploy', 'undeploy', 'status', 'health-check', 'logs', 'test'],
                               help='Staging environment action')
    staging_parser.add_argument('--kubeconfig', help='Path to kubeconfig file')
    staging_parser.add_argument('--namespace', default='netskope-staging', help='Kubernetes namespace')
    staging_parser.add_argument('--service', help='Service name for logs')
    staging_parser.add_argument('--follow', '-f', action='store_true', help='Follow logs')
    staging_parser.add_argument('--test-type', choices=['integration', 'e2e', 'security', 'performance', 'load'],
                               help='Type of tests to run')
    staging_parser.add_argument('--confirm', action='store_true', help='Skip confirmation prompts')
    
    # Production environment commands (READ-ONLY)
    production_parser = subparsers.add_parser('production', help='Production environment monitoring (READ-ONLY)')
    production_parser.add_argument('production_action',
                                  choices=['health-check', 'metrics', 'report', 'monitor', 'status'],
                                  help='Production monitoring action')
    production_parser.add_argument('--config', default='config/production.yaml',
                                  help='Production configuration file')
    production_parser.add_argument('--output', help='Output file for reports')
    production_parser.add_argument('--interval', type=int, default=300,
                                  help='Monitoring interval in seconds (for continuous monitoring)')
    production_parser.add_argument('--duration', type=int, default=3600,
                                  help='Monitoring duration in seconds (for continuous monitoring)')
    
    # Test commands
    test_parser = subparsers.add_parser('test', help='Run tests')
    test_parser.add_argument('test_type', nargs='?',
                            choices=['unit', 'integration', 'e2e', 'security', 'performance'],
                            help='Type of tests to run')
    test_parser.add_argument('-e', '--environment',
                            choices=['mock', 'local', 'integration', 'staging', 'production'],
                            help='Environment to run tests in')
    test_parser.add_argument('--html-report', action='store_true',
