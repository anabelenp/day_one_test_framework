...
class EnvironmentManager:
    ...
    def detect_environment(self) -> Environment:
        ...
        if self._is_local_development_environment():
            self.logger.info("Local development environment detected (Docker Compose services)")
            return Environment.LOCAL
    ...
    def load_configuration(self, environment: Optional[Environment] = None) -> EnvironmentConfig:
        """
    ...
    def get_service_config(self, service_name: str, environment: Optional[Environment] = None) -> ServiceConfig:
        """
    ...
    def get_environment_info(self, environment: Optional[Environment] = None) -> Dict[str, Any]:
        """
    ...
    def _create_environment_config(self, environment: Environment, config: Dict[str, Any]) -> EnvironmentConfig:
    ...
    def _validate_service_connectivity(self, config: EnvironmentConfig) -> bool:
        ...
        if config.environment != Environment.LOCAL:
            services_to_check.append(("Netskope API", config.netskope_api.host, config.netskope_api.port))
        
        self.logger.info(f"Validating connectivity for {len(services_to_check)} services...")
    ...
    def _check_port_connectivity(self, host: str, port: int, timeout: int = 10) -> bool:
        ...
        import socket
        try:
            self.logger.debug(f"Creating socket connection to {host}:{port} with timeout {timeout}s")
            ...
            self.logger.error(f"Exception during connectivity check for {host}:{port}: {e}")
            return False
