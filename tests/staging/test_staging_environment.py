...
class TestStagingEnvironment:
    ...
    def setup_staging_environment(self):
        ...
        if current_env != Environment.STAGING:
            pytest.skip(f"Staging tests require staging environment, got {current_env.value}")
    ...
    def test_environment_configuration(self):
        ...
        assert "ha" in config.redis.host.lower() or "sentinel" in config.redis.host.lower()
        ...
        assert config.netskope_api.host is not None
    ...
    def test_high_availability_services(self):
        ...
        for service, is_healthy in health_results.items():
            assert is_healthy, f"HA service {service} should be healthy in staging environment"
    ...
    def test_redis_ha_connectivity(self):
        ...
        # Set and get with HA
        assert cache_client.set(test_key, test_value), "Should be able to set value in HA Redis"
        retrieved_value = cache_client.get(test_key)
        assert retrieved_value == test_value, "Retrieved value should match set value in HA Redis"
        ...
        # Clean up
        assert cache_client.delete(test_key), "Should be able to delete key from HA Redis"
    ...
    def test_kafka_ha_connectivity(self):
        ...
        assert message_client.create_topic(test_topic), "Should be able to create HA topic"
        
        # Publish message to HA cluster
        assert message_client.publish(test_topic, test_message), "Should be able to publish to HA Kafka"
        ...
        assert received_message["test"] == test_message["test"], "Message content should match"
    ...
    def test_mongodb_ha_connectivity(self):
        ...
        # Clean up
        delete_result = db_client.delete_one(test_collection, {"name": "staging_ha_test"})
    ...
    def test_enhanced_api_connectivity(self):
        ...
        auth_result = api_client.authenticate({"api_key": "staging-api-key-2024-secure-enhanced"})
        ...
        for endpoint in enhanced_endpoints:
            ...
            if isinstance(response, dict):
                assert "data" in response or "error" not in response, f"Enhanced response from {endpoint} should be valid"
                ...
                if "data" in response and isinstance(response["data"], list) and len(response["data"]) > 0:
                    ...
                    if isinstance(first_item, dict):
                        ...
                        assert has_staging_indicator, f"Response should contain staging-specific data"
    ...
    def test_enhanced_monitoring_stack(self):
        ...
        if prometheus_url:
            try:
                ...
                metrics_response = requests.get(f"{prometheus_url}/api/v1/query?query=up", timeout=15)
                assert metrics_response.status_code == 200, "Should be able to query Prometheus HA"
                
                metrics_data = metrics_response.json()
                assert metrics_data.get("status") == "success", "Prometheus query should succeed"
                
            except requests.RequestException:
                pytest.skip("Prometheus HA not accessible (may be expected in some setups)")
        ...
        if grafana_url:
            try:
                ...
                pytest.skip("Grafana HA not accessible (may be expected in some setups)")
    ...
    def test_performance_characteristics(self):
        ...
        assert redis_write_time < 10.0, f"Redis HA writes should be reasonably fast, took {redis_write_time:.2f}s"
        assert redis_read_time < 8.0, f"Redis HA reads should be reasonably fast, took {redis_read_time:.2f}s"
        ...
        assert api_response_time < 20.0, f"Enhanced API responses should be reasonably fast, took {api_response_time:.2f}s"
    ...
    def test_data_persistence_and_durability(self):
        ...
        assert cache_client.get(persistence_key) == persistence_value, "Redis HA data should persist"
        
        found_doc = db_client.find_one(persistence_collection, {"test_id": "ha_persistence_test"})
        assert found_doc is not None, "MongoDB HA document should persist"
        assert found_doc["data"] == persistence_doc["data"], "MongoDB HA data should match"
        assert found_doc["ha_replicated"] == True, "HA replication flag should be preserved"
    ...
    def test_high_availability_resilience(self):
        ...
        for key in resilience_keys:
            ...
            assert value == f"resilient_value_{key}", f"HA Redis should return correct value for {key}"
        ...
        for doc in resilience_docs:
            found_doc = db_client.find_one(resilience_collection, {"doc_id": doc["doc_id"]})
            assert found_doc is not None, f"HA MongoDB should maintain access to {doc['doc_id']}"
            assert found_doc["data"] == doc["data"], f"HA MongoDB should return correct data"
    ...
    def test_concurrent_load_handling(self):
        ...
        def concurrent_operations(thread_id: int, operation_count: int = 100):
            ...
            try:
                for i in range(operation_count):
                    ...
                    found_doc = db_client.find_one("ha_load_test", {"thread_id": thread_id, "operation": i})
                    assert found_doc is not None
                    assert found_doc["ha_load_test"] == True
                    
                    db_client.delete_one("ha_load_test", {"thread_id": thread_id, "operation": i})
        ...
        assert total_errors / max(total_operations, 1) < 0.05, "Error rate should be less than 5% in HA environment"
        assert avg_duration < 60, "Average duration should be reasonable for HA environment"
...
class TestStagingEnvironmentSecurity:
    ...
    def setup_staging_environment(self):
        ...
        if current_env != Environment.STAGING:
            pytest.skip(f"Staging security tests require staging environment, got {current_env.value}")
    ...
    def test_enhanced_authentication(self):
        ...
        try:
            ...
            token_response = api_client.post("/api/v2/auth/token", {
    ...
    def test_tls_security(self):
        ...
        if config.security.get("tls_enabled", False):
            ...
            assert "https" in config.netskope_api.host.lower(), "API should use HTTPS in staging"
    ...
    def test_audit_logging(self):
        ...
        try:
            audit_logs = db_client.find_one("audit_logs", {
