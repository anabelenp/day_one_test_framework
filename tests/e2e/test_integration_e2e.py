...
class TestIntegrationE2E:
    ...
    def setup_integration_environment(self):
        ...
        if current_env != Environment.INTEGRATION:
            pytest.skip(
                f"E2E tests require integration environment, got {current_env.value}"
            )
        ...
        auth_result = self.api_client.authenticate(
            {"api_key": "integration-api-key-2024"}
        )
    ...
    def test_complete_security_event_workflow(self):
        ...
        assert consumed_event is not None, (
            "Should find our security event in consumed messages"
        )
        assert consumed_event["event_type"] == security_event["event_type"], (
            "Consumed event should match"
        )

        # Step 7: Verify event can be retrieved from database
        stored_event = self.db_client.find_one(
            collection, {"event_id": security_event["event_id"]}
        )
        assert stored_event is not None, "Should retrieve event from database"
        assert stored_event["severity"] == security_event["severity"], (
            "Stored event should match"
        )
    ...
    def test_policy_management_workflow(self):
        ...
        # cache_client.get() already returns parsed JSON, no need to json.loads()
        assert not cached_policy["enabled"], "Cached policy should be disabled"

        stored_policy = self.db_client.find_one(
            collection, {"policy_id": policy["policy_id"]}
        )
    ...
    def test_user_risk_assessment_workflow(self):
        ...
        assert updated_user["risk_score"] == new_risk_score, (
            "Risk score should be updated"
        )

        cached_user = self.cache_client.get(cache_key)
        assert cached_user is not None, "Should retrieve cached user"
        # cache_client.get() already returns parsed JSON, no need to json.loads()
        assert cached_user["risk_score"] == new_risk_score, (
            "Cached risk score should be updated"
        )
        ...
        for event in security_events:
            self.db_client.delete_one(
                events_collection, {"event_id": event["event_id"]}
            )
    ...
    def test_alert_generation_and_notification_workflow(self):
        ...
        updated_alert = self.db_client.find_one(
            alerts_collection, {"alert_id": alert["alert_id"]}
        )
        ...
        assert updated_alert["assigned_to"] == alert["assigned_to"], (
            "Alert should be assigned"
        )
        ...
        self.db_client.delete_one(
            events_collection, {"event_id": critical_event["event_id"]}
        )
    ...
    def test_system_performance_under_load(self):
        ...
        def concurrent_operations(thread_id: int) -> Dict[str, Any]:
            ...
            try:
                for i in range(20):  # 20 operations per thread
                    ...
                    found_doc = self.db_client.find_one(
                        "load_test", {"thread_id": thread_id, "operation": i}
                    )
        ...
        assert total_errors / total_operations < 0.1, (
    ...
    def test_data_consistency_across_services(self):
        ...
        test_data = {
            ...
            "value": 12345,
            "timestamp": time.time(),
            "metadata": {"test_type": "consistency", "environment": "integration"},
        }
        ...
        assert consumed_data["value"] == test_data["value"], (
            "Consumed value should match"
        )
