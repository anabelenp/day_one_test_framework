...
class TestIntegrationEnvironment:
    ...
    def setup_integration_environment(self):
        ...
        if current_env != Environment.INTEGRATION:
            pytest.skip(f"Integration tests require integration environment, got {current_env.value}")
    ...
    def test_environment_configuration(self):
        ...
        assert config.netskope_api.port > 0
    ...
    def test_service_health_checks(self):
        ...
        for service, is_healthy in health_results.items():
            assert is_healthy, f"Service {service} should be healthy in integration environment"
    ...
    def test_kafka_connectivity(self):
        ...
        # Publish message
        assert message_client.publish(test_topic, test_message), "Should be able to publish message"
    ...
    def test_mongodb_connectivity(self):
        ...
        assert found_doc["name"] == test_document["name"], "Document content should match"
        ...
        # Delete document
        delete_result = db_client.delete_one(test_collection, {"name": "integration_test"})
    ...
    def test_mock_api_connectivity(self):
        ...
        for endpoint in endpoints_to_test:
            ...
            assert "data" in response or "error" not in response, f"Response from {endpoint} should be valid"
    ...
    def test_monitoring_stack_accessibility(self):
        ...
        if prometheus_url:
            try:
                ...
                pytest.skip("Prometheus not accessible (may be expected in some setups)")
    ...
    def test_service_discovery(self):
        ...
        redis_config = self.env_manager.get_service_config("redis", Environment.INTEGRATION)
        kafka_config = self.env_manager.get_service_config("kafka", Environment.INTEGRATION)
        mongodb_config = self.env_manager.get_service_config("mongodb", Environment.INTEGRATION)
        api_config = self.env_manager.get_service_config("netskope_api", Environment.INTEGRATION)
        ...
        if self.env_manager._is_kubernetes_environment():
            assert ".svc.cluster.local" in redis_config.host or "service" in redis_config.host, \
                "Redis should use Kubernetes service discovery"
            assert ".svc.cluster.local" in kafka_config.host or "service" in kafka_config.host, \
                "Kafka should use Kubernetes service discovery"
            assert ".svc.cluster.local" in mongodb_config.host or "service" in mongodb_config.host, \
    ...
    def test_data_persistence(self):
        ...
        # Verify data exists
        assert cache_client.get(persistence_key) == persistence_value, "Redis data should persist"
        
        found_doc = db_client.find_one(persistence_collection, {"test_id": "persistence_test"})
    ...
    def test_performance_characteristics(self):
        ...
        assert redis_write_time < 5.0, f"Redis writes should be fast, took {redis_write_time:.2f}s"
        assert redis_read_time < 5.0, f"Redis reads should be fast, took {redis_read_time:.2f}s"
        ...
        assert api_response_time < 10.0, f"API responses should be fast, took {api_response_time:.2f}s"
    ...
    def test_error_handling(self):
        ...
        invalid_doc = db_client.find_one("non_existent_collection", {"invalid": "query"})
...
class TestIntegrationEnvironmentSecurity:
    ...
    def setup_integration_environment(self):
        ...
        if current_env != Environment.INTEGRATION:
            pytest.skip(f"Integration security tests require integration environment, got {current_env.value}")
