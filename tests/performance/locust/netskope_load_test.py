...
pytest.skip(
    "This is a Locust load test file, not a pytest test. Run with: locust -f tests/performance/locust/netskope_load_test.py"
)
...
class NetskopeSecurityUser(HttpUser):
            ...
            if response.status_code == 200:
                try:
                    ...
                    if "action" in result:
                        response.success()
                        logger.debug(
                            f"SWG check for {test_url}: {result.get('action', 'unknown')}"
                        )
                    else:
            ...
            if response.status_code == 200:
                try:
                    ...
                    if "scan_result" in result:
                        response.success()
                        logger.debug(
                            f"DLP scan for {filename}: {result.get('scan_result', 'unknown')}"
                        )
                    else:
    ...
    def check_access_ztna(self):
        ...
        payload = {
            ...
            "mfa_status": random.choice([True, False])
            if app["mfa_required"]
            else False,
            ...
            if response.status_code == 200:
                try:
                    ...
                    if "access_decision" in result:
                        response.success()
                        logger.debug(
                            f"ZTNA access for {app['name']}: {result.get('access_decision', 'unknown')}"
                        )
                    else:
            ...
            if response.status_code == 200:
                try:
                    ...
                    if "action" in result:
                        response.success()
                        logger.debug(
                            f"Firewall check for {traffic['protocol']}:{traffic['port']}: {result.get('action', 'unknown')}"
                        )
                    else:
                        response.failure("Missing action in response")
                except json.JSONDecodeError:
...
class AdminUser(HttpUser):
    ...
    def get_security_events(self):
        ...
        with self.client.get(
            "/api/v2/events", params=params, name="Admin - Security Events"
        ) as response:
            if response.status_code == 200:
                logger.debug(
                    f"Retrieved {len(response.json().get('events', []))} security events"
                )
    ...
    def get_security_reports(self):
        ...
        with self.client.get(
            "/api/v2/reports", params=params, name="Admin - Security Reports"
        ) as response:
            if response.status_code == 200:
    ...
    def get_system_health(self):
        ...
        with self.client.get(
            "/api/v2/health", name="Admin - System Health"
        ) as response:
            if response.status_code == 200:
...
if __name__ == "__main__":
    ...
    if hasattr(scenarios, f"{scenario_name.upper()}_LOAD") or hasattr(
        scenarios, f"{scenario_name.upper()}_TEST"
    ):
        scenario = getattr(
            scenarios,
            f"{scenario_name.upper()}_LOAD",
        ...
        cmd = [
            ...
            "--headless",
            "--csv",
            f"reports/locust_{scenario_name}_results",
        ]
